#!/usr/bin/env bash
# 发布一个版本到 GitHub Releases：上传 .dmg + ModelLink.app.tar.gz + latest.json。
...
TARGZ="$BUNDLE_MACOS/ModelLink.app.tar.gz"
DMG="$(ls "$BUNDLE_DMG"/ModelLink_*_aarch64.dmg 2>/dev/null | head -1 || true)"
...
NOTES_ARG=(--notes "ModelLink $TAG")
[ -f RELEASE_NOTES.md ] && NOTES_ARG=(--notes-file RELEASE_NOTES.md)

echo "→ 发布 $TAG 到 Win-Hao/ModelLink ..."
# shellcheck disable=SC2086
gh release create "$TAG" \
  --repo Win-Hao/ModelLink \
  --title "ModelLink $TAG" \
  --latest \
  "${NOTES_ARG[@]}" \
