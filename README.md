...
- **Claude 里直接显示厂商模型名**：模型选择器显示 `Kimi-k2.6` 这样的真名，不再只有 claude-3-* 槽位名（需较新版本 Claude Desktop）
- **自动保存 + 应用状态提醒**：改没改、生效没生效，页头一眼看清；「应用到 Claude Desktop」仍是唯一按钮
- **应用内自动更新**：从 2.0 起新版本自动提醒、一键安装（macOS）
- **代理端口可配置**：默认 5678；被其他程序占用时可在设置页直接更换，立即生效
...
- 支持同时配置多个 API 服务商（模型总数最多 8 个），内置主流服务商预设（带品牌图标），也可自定义
- 支持 1M 上下文模型变体、按服务商设置推理强度、代理端口可配置
