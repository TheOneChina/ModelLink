...
- **双栏**：左列 196px 服务商清单 + 右侧编辑器卡。
  ...
  - 编辑器：
    - 「API 地址」「API 密钥」两字段（1.2fr : 1fr 网格）；密钥为密码框 + 眼睛图标切换明文。
    - 模型区标签行：「模型 · 右侧为 Claude 中显示的名称」+ 右侧「测试连接」outline 按钮与 inline 结果（§9）。
...
- **测试连接**：点击后按钮 loading（Loader2 spin）→ inline 结果：绿「连接成功 (HTTP 200)」或红色错误信息（后端话术），6 秒后淡出。
